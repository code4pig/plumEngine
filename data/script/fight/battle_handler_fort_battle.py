...
class BattleHandlerFortBattle(BattleHandlerBase):
    """
    占领战要塞据点战斗处理
    """
    def __init__(self):
        ...
        self.field_id = None                # 区域id
        self.block_id = None                # 区块id
        self.point_id = None                # 据点id
        self.defense_clan_id = None         # 防守公会id
        self.attack_clan_id = None          # 攻击公会id
    ...
    def init_battle(self, server_group_id, battle_unique_id, field_id, block_id, point_id, defense_clan_id, attack_clan_id,
                    battle_start_time, battle_end_time, total_hp, rest_hp):
        self.server_group_id = server_group_id
        self.battle_unique_id = battle_unique_id
        self.field_id = field_id
        self.block_id = block_id
        self.point_id = point_id
        self.defense_clan_id = defense_clan_id
        self.attack_clan_id = attack_clan_id
    ...
    def handle_sec_timer(self, current_time):
        if self.is_finish:
            ...
            if (current_time - self.last_write_back_time >= WRITE_BACK_INTERVAL and self.last_damage_time > self.last_write_back_time) or current_time >= self.end_time:
                # 有新伤害需要回写 或 时间到已结束
                ModObjFac.CreateApp().get_fight_2_db_rpc().FortBattleWriteBack(self.server_group_id, self.battle_unique_id, self.field_id, self.block_id,
                                                                               self.defense_clan_id, self.attack_clan_id, self.point_id, self.rest_hp, self.user_damage_dict)
                self.user_damage_dict.clear()
    ...
    def handle_user_leave(self, server_user_id):
        ...
        ModObjFac.CreateApp().get_fight_2_db_rpc().FortBattleUpdateBattleUser(self.server_group_id, self.field_id, self.block_id, self.point_id, len(self.battle_user_dict))
    ...
    def _handle_damage(self, server_user_id, msg):
        ...
        if not self.is_finish and self.rest_hp > 0 and recv_obj.damage > 0:
            ...
            if self.rest_hp == 0:
                ...
                # 回写一次数据
                ModObjFac.CreateApp().get_fight_2_db_rpc().FortBattleWriteBack(self.server_group_id, self.battle_unique_id, self.field_id, self.block_id,
                                                                               self.defense_clan_id, self.attack_clan_id, self.point_id, self.rest_hp,
                                                                               self.user_damage_dict, server_user_id)
                self.user_damage_dict.clear()
