...
namespace application
{
	...
	void CAsioService::StartNetwork(int32 nettype, const char* ip, int32 port, uint16 limit_msg_num, uint8 limit_invalid_num, bool encrypt, int32 maxclients)
	{
		if (GetNetServer(nettype) == NULL)
		{
		...
		GetNetServer(nettype)->Start(ip, port, limit_msg_num, limit_invalid_num, encrypt, maxclients);
	}
